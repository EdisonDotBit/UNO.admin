...
// if ($_SESSION['dbname'] === "" || $_SESSION['tblname'] === null ) {
//     header("location: front_page.php");
// }
...
<html>
    ...
    <body>
        ...
        <div class="main_nav">
            ...
            <button class="main_nav_btn top_btn_c active"><a href="">INSERT</a></button>
        ...
        <div class="main_ui">
            ...
            <div class="insert">
                    <form action="main.php" method="post">
                        ...
                        <select onchange="selectdb(this)" name="dbname" class="column_input">';
            if ($_SESSION['dbname'] == "") {
                echo '
                            <option value="">...</option>
                ';
            ...
            echo'
                        ...
                        <b class="create_title">Table: </b>    
                        <select name="tblname" id="tblname" style="width: 200px;" onchange="selectTbl(this)" class="column_input">
                ';
            if ($_SESSION['tblname'] == "") {
                echo '
                            <option value="">...</option>
            ...
            if ($_SESSION['dbname'] != "") {
                $current    = $_SESSION['dbname'];
                $query      = "SHOW TABLES FROM `$current`";
                $fetch_tbl  = mysqli_query($key, $query);
                while ($row = mysqli_fetch_assoc($fetch_tbl)){
                    $tbl = $row['Tables_in_'.strtolower($current)];
                    if($tbl == $_SESSION['tblname']){
                        echo '  <option value="'.$tbl.'" selected="selected">'.$tbl.'</option>';
                    } else {
            ...
            if ($_SESSION['tblname'] == "") {
                echo '
                        ...
                        <p class="insert_title">Please Select a Table</p>
                ';
            } else {
            echo'
                        </select>
                        <p class="insert_title">Insert Data: </p>
                ';
                ...
                while($columns  = mysqli_fetch_assoc($fetch)) {
                    if ($columns['Extra']!="auto_increment"){
                        ...
                        if ($columns['Type'] == 'date') {
                            echo '
                                <p class="insert_label">'.$current.': </p>
                                <input name="'.str_replace(' ','_',$current).'" type="date"  class="insert_input" required><br>
                            ';
                        } else {
                            echo '
                                <p class="insert_label">'.$current.': </p>
                                <input name="'.str_replace(' ','_',$current).'" class="insert_input" placeholder="Type here" required><br>
                            ';
                        }
                    }
                }
            ...
            echo '
                        <button type="reset" class="create_btn reset_btn">Reset</button>
                        <button name="insert" class="create_btn insert_btn">Insert</button>
                </form>
        ...
        <script>
            ...
            function selectTbl(t) {
                var value = t.value;
                if (value == ""){
                } else {
                    window.top.location.href = 'main.php?newtable='+value;
                }
            }
            ...
            function selectdb(t) {
                ...
                if (value == ""){
                    ...
                    window.top.location.href = 'main.php?newdb='+value;
                }
            }
